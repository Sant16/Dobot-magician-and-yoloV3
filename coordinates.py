...
def dobot_coor(frame, x_cor, y_cor, w_, h_):
    ...
    for r in range(len(x_cor)):
        ...
        y_ = int(y_*10) 
        x_ = int(((x_ * (cm_cubos*10))/px_cubos) + 125)  
        obx.append(x_)
        oby.append(y_)
        print(f'{obx}/{oby}')
...
def dobot_mov(obx, oby):
    ...
    for i in range(len(obx)):
        ...
        device.move_to(x_, y_, 0, 90, wait=False)  # we wait unt
        device.move_to(x_, y_, -39, 90, wait=True)  # we wait unt
        device.grip(True)
        device.move_to(x_, y_, 10, 0, wait=False)  # we wait unt
        #device.move_to(offset, 0, 10, 90, wait=True)  # we wait unt
        #device.move_to(offset, 0, -17, 90, wait=True)  # we wait unt
        device.grip(False)
        #device.move_to(offset, 0, 10, 90, wait=True)  # we wait unt
        offset = offset - 35
